---
title: "Physiological Arousal as a Predominant Source of Individual Differences in Functional Brain Networks"
collection: publications
category: manuscripts
permalink: /publication/6-ur-natneur
excerpt: 'Sources of individual variability in functional brain network properties are poorly understood, yet such individual differences are promising targets for fMRI-based biomarkers in precision psychiatry. Using the systemic low frequency oscillations (sLFO), we assess arousal x functional network relationships on large-scale and identify sLFO-assessed arousal as a predominant source of individual differences in brain function.'
date: '2025-10-02'
hide_year: true
venue: bioRxiv; In Submission at Nature Neuroscience
#slidesurl:
#paperurl: 'http://hilljua.github.io/files/xiao_2025_JAMAOPEN.pdf'
citation: 'Hill J, Zhai T, Korponay C, Welsh JC, Salmeron BJ, Ross T, Yang Y, Frederick B deB, Janes A. (In submission at Nature Neuroscience). Physiological Arousal as a Predominant Source of Individual Differences in Functional Brain Networks.'
---
